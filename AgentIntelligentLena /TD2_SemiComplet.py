...
def Salete(Matrice,PourcentageS,N):
    ...
    while i < NbSalete : #Tant que le nombre de cases sales est inf au nombre souhaité 
        ...
        if (Matrice[L][C] == '0') : #Si a cette postion il n'a ni deja de la salte , ni un obstacle alors 
            ...
            ListeCaseSale.append((L,C))
            i += 1
    return (Matrice,NbSalete,ListeCaseSale)
...
#         print(type(Proba))
#         print(Proba)
        if Proba == 0 : #l'aspiration echoue 
            print("Il semblerai que l'aspiration est echoué sur la case " + str(PosAspi) + " Re-essayons.")
            Reponse = 'Moyen'
        else : 
    ...
    else : Reponse = False #Case deja propre 
    #print(M)
    return (Reponse)
...
def Droite(PosAspi): #verifie si l'aspirateur peux tourner a droite ou si il se prend le mur
    Proba = int(random.choices([0, 1], weights=[0.1, 0.9])[0]) #10% pour le deplacement echoue
    if Proba == 1 : 
        PosAspi = (PosAspi[0],PosAspi[1]+1)
        print("L'aspirateur est allé à droite. Sur la case" , PosAspi)
    else : print("L'aspirateur a glisse, il n'a pas pu changer de position. Il est toujours sur la case" , PosAspi)
    return (PosAspi)  
#Droite((3,4))
...
def ActualiserEtat(Dict,PosAspi) : #Prend en argument l'ancienne etat et la positon actuelle
    Dict['Position'] = PosAspi #Changement de position 
    for i in Dict['Sale'] : #On prend une case sale
        if i < Dict['Position'] : # Si elle est inferieur a la position actuel alors l'aspi l'a deja netoye
            Dict['Sale'].remove(i) #dDonc elle est plus sale
            Dict['Propre'].append(i) # Masi propre
    Dict['Propre'].sort()
    return Dict
...
def AgentReflexeAvecEtat(UniteT,N,PourcentageS) : #SE PREND PAS LES MURS ET RETIENT SA POSTITION 
    ...
    ListeCasesSales = SaleteF[2] #Liste des cases sales 
    NbAspiration=0
    NbDeplacement =0
    ...
    if (MatriceS != MatriceP(N)): 
        ...
        ' EN PLUS  ==> TD2 ' 
        Dict = {}
        Dict['Position'] = PosAspi
        ListeCasesSales.sort()
        Dict['Sale'] = ListeCasesSales
        ListeCasesPropres = []
        for i in range (N) :
            for j in range (N) :
                if (i,j) not in Dict['Sale'] :
                    ListeCasesPropres.append((i,j))
        Dict['Propre'] = ListeCasesPropres
        print (Dict)
    
        
        #Verification que la case (0,0) est propre
        Response = VerifAspiration(MatriceS,PosAspi)
        if Response != False : #Si sale
            while Response != True : #tant que l'aspiration ne marche pas 
                NbAspiration +=1
                Response = VerifAspiration(MatriceS,PosAspi)
            NbAspiration +=1 
        ...
        while (MatriceS != MatriceP(N)): #Tant que la salle est sale
            if ((NbDeplacement+NbAspiration)<UniteT) : # et si bien sur l'aspirateur est encore viable
                ...
                Proba = int(random.choices([0, 1], weights=[0.05, 0.95])[0]) #05% pour que 1 case devienent sale
                if Proba == 0 :
                    L,C = (random.randint(0,N-1),random.randint(0,N-1))
                    if MatriceS[L][C] == '0' :
                        MatriceS[L][C] = '1'
                        print("Attention, une nouvelle case est devenue sale" , (L,C))
                        Afficher(MatriceS,PosAspi,N) # Pour afficher la nouvelle Sale.
                        # Le Dict se modifie aussi
                        
                        #Si L'aspirateur est deja passe sur cette case alors il va directement sur cette case pour la nettoyer :
                        #Il n'y a pas d'obsatcle donc simple :
                        if (L,C) < PosAspi :
                            NbDeplacement += (PosAspi[0] - L) + (PosAspi[1] - C)
                            #Verifiaction 
                            Response = VerifAspiration(MatriceS,(L,C))
                            if Response != False : #Si sale
                                while Response != True : #tant que l'aspiration ne marche pas 
                                    NbAspiration +=1
                                    Response = VerifAspiration(MatriceS,(L,C))
                        ...
                        #Sinon il va tomber naturelement sur cette nouvelle case. 
                    #si case deja sale , rien . 
...
def choix() :
    ...
    AgentReflexeAvecEtat(1000,N,PourcentageS)
